...
export class UploadController {
  ...
  @Post('person')
  @ApiOperation({ summary: 'Subir foto de persona o tarjeta de identificación' })
  ...
  async uploadPersonImage(
    ...
    const thumbnailUrl = this.uploadService.getThumbnailUrl(result.publicId, 200, 200);
  ...
  async uploadResourceImage(
    ...
    const thumbnailUrl = this.uploadService.getThumbnailUrl(result.publicId, 150, 150);
  ...
  async uploadCampImage(
    ...
    const thumbnailUrl = this.uploadService.getThumbnailUrl(result.publicId, 300, 200);
  ...
  @Post('badge')
  @ApiOperation({ summary: 'Subir imagen de badge/logro/medalla (asset genérico)' })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    schema: {
      type: 'object',
  ...
  @UseInterceptors(FileInterceptor('file'))
  async uploadBadge(@UploadedFile() file: Express.Multer.File): Promise<UploadResponseDto> {
    if (!file) {
      throw new BadRequestException('No se proporcionó ningún archivo');
    }
  ...
  @Post('multiple')
  @ApiOperation({ summary: 'Subir múltiples imágenes (ej: evidencia de exploraciones)' })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    schema: {
      type: 'object',
  ...
  async deleteImage(@Body('publicId') publicId: string): Promise<{ message: string }> {
    if (!publicId) {
      throw new BadRequestException('Debe proporcionar el publicId de la imagen');
    }
